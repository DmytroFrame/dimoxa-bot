...
from models.functions import getSettings
...
def sendCommand(listCommand, interval=0):
    def asyncSender(listCommand, interval):
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            sock.connect((getSettings("rconData")['address'], getSettings("rconData")['port']))
        except:
            sock.connect((getSettings("rconData")['address'], getSettings("rconData")['port']))


        login(sock, getSettings("rconData")['password'])

        for commands in listCommand:
            command(sock, commands)
            sleep(interval)

        sock.close()

    taskSendCommand = Thread(target=asyncSender, args=(listCommand, interval,))
    taskSendCommand.start()